...
public class UserDao {
    ...
    public void updateUserAddress(User user) throws SQLException {
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
        String sql ="update user set username = ?, email = ?, name = ?,phone=?,address=? ,isadmin = ?, isvalidate = ? where id = ?";
        r.update(sql,user.getUsername(),user.getEmail(),user.getName(),user.getPhone(),user.getAddress(),user.isIsadmin(),user.isIsvalidate(),user.getId());
    }
